<script lang="ts">
    ...
    const startFetch = () => {
        fetching = true;
        $reviews.fetchReviews($auth.account).finally(() => {
            fetching = false;
        });
    };
...
<footer class="fixed bottom-0 left-0 dark:bg-gray-900 bg-gray-50 shadow-md w-full px-2 py-2">
    <div class="flex justify-between">
        <div class="p-2 flex items-center">
            <Image src={$auth.account?.user?.avatar} class="h-6 w-6 flex-shrink-0 rounded-full">
                <svg
                    slot="error"
            ...
            <span
                class={`${
                    $auth.account?.user?.url ? 'cursor-pointer hover:text-slate-600/70 dark:hover:text-white/70' : ''
                } ml-1 block truncate`}
                on:click={() => ($auth.account?.user?.url ? open($auth.account?.user?.url) : null)}
                >{$auth.account?.user?.name}</span
            >
        </div>
        <div class="flex justify-between">
            <button
