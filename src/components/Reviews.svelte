<script lang="ts">
    ...
    const interval = setInterval(() => {
        $reviews.fetchReviews($auth.account);
        dateUpdater++;
    }, $auth.settings.fetchInterval || defaultSettings.fetchInterval);

    onMount(() => {
        $reviews.fetchReviews($auth.account).finally(() => (loading = false));
    });
                ...
                >
                    <div class="flex">
                        ...
                        <div class="whitespace-nowrap text-ellipsis overflow-hidden w-full">
                            <span class="text-slate-500 dark:text-gray-300">{review.repository}</span>
                            <span
                            ...
                            {#key dateUpdater}
                                <span class="text-sm text-gray-500 truncate dark:text-gray-400">
                                    #{review.number} opened
                                    {timeAgo(review.createdAt)} by @{review.author}
                                </span>
                            {/key}
                        </div>
                    </div>
